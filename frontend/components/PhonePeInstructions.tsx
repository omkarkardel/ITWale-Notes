...
type Props = { info: PhonePeInfo; onClose?: () => void }
...
export default function PhonePeInstructions({ info, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(info.upiIntentUrl, { width: 240 })
      .then((url: string) => { if (isMounted) setQr(url) })
      .catch((err: unknown) => { console.error('Failed to build QR code', err) })
    ...
    <Card className="border-blue-500 bg-blue-50">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-blue-700/80">Access has been granted instantly. Please complete the PhonePe payment using any option below. These details are also emailed to you.</p>
          <ul className="text-sm text-blue-900 space-y-1">
            <li><strong>UPI ID:</strong> {info.upiId}</li>
            <li><strong>PhonePe mobile:</strong> {info.phoneNumber}</li>
            {info.payeeName ? (<li><strong>Payee name:</strong> {info.payeeName}</li>) : null}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            {onClose ? (<Button variant="ghost" onClick={onClose}>Dismiss</Button>) : null}
          </div>
        </div>
