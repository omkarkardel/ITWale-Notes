...
export default function EndsemPyq({ params }: { params: { subjectId: string } }) {
  const { subjectId } = params
  const [items, setItems] = useState<any[]>([])
  const [phonePeInfo, setPhonePeInfo] = useState<PhonePeInfo | null>(null)
  const [successMessage, setSuccessMessage] = useState<string | null>(null)
  useEffect(() => { (async () => {
    const res = await fetch('/api/resources/list', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subjectId, type: 'PAPER', examType: 'ENDSEM', hasFile: true }) })
  ...
  return (
    <main>
      <h2 className="text-xl font-semibold mb-4">Endsem PYQ Papers</h2>
