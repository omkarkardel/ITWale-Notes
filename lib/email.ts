...
export async function sendMail(opts: { to: string; subject: string; html?: string; text?: string }): Promise<{ messageId?: string; previewUrl?: string } | void> {
  ...
  if (!useSmtp) {
    // Default: use Ethereal test inbox (no real email delivery, but preview URLs)
    ...
    const looksPlaceholder =
      ...
      /example\.com$/i.test(String(host)) ||
      /@example\.com$/i.test(String(user)) ||
      String(pass).toLowerCase() === 'password'
    if (looksPlaceholder) {
